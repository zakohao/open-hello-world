...
\todo[inline]{先に説明，あとで「まとめると」では全く理解できないので，最初に「まとめると」の内容をもってきて，後でわかりにくいところを説明に変更してください}

本研究に用いるジョブセットの生成手続きを以下に述べる．
まず最初に
\begin{enumerate}
...
\end{enumerate}
の二つの行列を用意する．
\todo[inline]{この二つの行列をどう準備したのか，その考え方，手続きについての記述が一切なく，根拠が不明}


\subsection{加工工程順序と加工時間構成}
加工工場は６種類の製品種別に生産が可能とする。６種類の製品種別ごとの加工工程順序を行列$OM$の形式で表す．行は製品種別，列は各製品の加工工程順番に対応する．
...
ここで$OM_{s,q} = \{1,2,\dots,12,13\}$ は製品$s$の第$q$番目加工工程の加工機械は$OM_{s,q}$の要素を意味する。
...
\todo[inlune]{この数値とした根拠が２つの行列について全く示されていない．書いたほうがいいです．}
...
受注数およびロット数のばらつきを表現するために、
受注数は平均 $\mu_{odder}=10$、分散 $\sigma^2_{odder}=2$ の正規分布に従う乱数で生成し、
ロット数は平均 $\mu_{lot}=3$、分散 $\sigma^2_{lot}=1$ の正規分布に従う乱数として生成する。
...
上記のパラメータ設定の平均や分散を用いて実際の生産現場に見られる受注量とロットサイズのランダムネスでデータとして生成する。
具体的なデータ生成はPythonで実装した.その手順を以下にまとめる.

\begin{description}
  \item[手順1:] 乱数 seed $seed_{value}$ を設定し，
  受注数およびロット数の正規分布パラメータ，
  ならびに機械別単位加工時間・基準加工時間パラメータを初期化する．

  \item[手順2:] 生成するジョブセット数を $gen=10000$ とし，
  $gen=0,\ldots,gen-1$ について手順2$\sim$8 を繰り返す．

  \item[手順3:] 受注数の候補として，
  平均 $\mu_{odder}$，分散 $\sigma^2_{odder}$ の正規分布から
  サンプルサイズ $odder_{size}$ 個の乱数系列を生成する．
  生成された乱数のうち $0$ 以下の値を除外し，
  残った候補の中から 1 つをランダムに選択して，
  実際の受注数 $odder_{number}$ とする．

  \item[手順4:] 同様に，ロット数を生成するために
  平均 $\mu_{lot}$，分散 $\sigma^2_{lot}$ の正規分布から
  ロット数候補を生成し，
  $0$ 以下を除外したロット数の候補集合を得る．

  \item[手順5:] 受注数 $odder_{number}$ と製品種類数 $6$ に基づき，
  オーダー原始データ行列 $odder$
  （サイズ $odder_{number} \times count_{type}$）を初期化する．
  各注文 $O_i$ と製品種別 $J_s$ について，二項分布に従い
  確率 $P=0.7$ で当該製品が注文に含まれるとし，
  含まれる場合にはロット数候補集合から 1 つをランダムに選択して
  $odder_{i,s}$ に代入する．

  \item[手順6:] 行列 $odder$ の列方向の合計に，
  ...
  \item[手順8:] 行列 $OM$ および $OT$ の数値を参照して，
  行列 $JOBSET$ を作成する．
...
ただし，同じジョブの作業について、同じ機械で連続的な二つの加工工程を加工しないように、作業の前作業と後作業のどれかの作業機械は一致しないと設定し，次の条件を加えている．
...
以上の手順で実際の生産現場に見られるランダムネスを持つ受注量とロットサイズのデータが生成される。

補足として，製品種別と処理時間の考え方について，説明する．
製品種別は確率 P = 0.7の二項分布に従い注文票に記載される．
一つの注文票の製品種別に重複はないものとし，
注文票別の製品種別行列 $odder$ を構成する.
ロット数の考え方は，
各注文票の製品種別ごとの総注文件数 $n_{s}$ をロット数とする.
...
\bf{最適化の実験ではこの10000のジョブセットから121個をランダムに選択した．これを1つの実験セットの単位として最適スケジュール案を探索する対象とする．}
\todo[inline]{これがもっとちゃんとわかるように書かないと，非常に重要な点です．また121のジョブセットを集めたデータに名称を付けないと，とても分かりにくいと思います．}


この実験セット単位に含まれる121ジョブセットの加工工程の加工時間の分散を計算し、
分散の大きさを基準として、ジョブセットの特徴を含まれる注文書の多様性で(小)、(並)(大)のように３タイプに分類する。
注文書の多様性が小さい（小）とは，比較的注文書内容の均一性が高く，一方で（大）であれば，注文書の内容のバラツキが大きく，生産スケジュールを計算する際に，非常に短時間で加工がおわる単純な加工依頼から，ほぼ全加工機械の処理を必要とし，ロット数の変動も大きなものまで対象に，様々な生産管理スケジュールを
検討しなくてはならない．計算機実験での最適化の評価は３タイプの属性
選択されたデータを次章の実験で利用する。
\todo[inline]{この大中小をどういう計算式で分類したかをここで書くほうがよい．}
\begin{figure}[htbp]
  \centering
...
\todo[inline]{ジョブという言葉が，ここから頻出するけど，ジョブセットは定義したけど，注文書単位で今まで記述してきたので，ジョブセットに含まれる製品要素（製品種別とロット数）を以下でジョブと呼ぶ．というような定義がいるのでは？途中で私が見落とした可能性もありますが．}

本研究では、JSSPに対して、ジョブごとの工程順序を厳密に保持しつつ、
複数の製品に属する加工工程の加工順序を染色体として表現する遺伝的アルゴリズム(GA)を用い、
解の改善を行う。

まず、注文書であるジョブセットの製品種別$s$の注文のジョブ $J_s$ の加工工程列は、機械割当行列 $OM$ および加工時間行列 $OT$ に基づく。
1 つのジョブは、「使用機械 $m_{s,q}$ と加工時間 $t_{s,q}$ の列からなる工程」として表現される。
まず染色体として、全ジョブの各工程を含む遺伝子列
\[
\text{genes} = \bigl[J_{s}F_{q},J_{s'}F_{q'}, \dots \bigr]
\]
を定義する。ここで各遺伝子は「ジョブ $J_{s}$ の第 $q$ 工程」を表す。
スケジュールの手順について、遺伝子列に従って工程を順に配置し、
同一製品ジョブ内の工程順序制約および各機械が同時に 1 工程しか処理できないという制約を満たすように
処理開始時刻と終了時刻を更新し、
スケジュール全体の総加工時間を算出する。

まず遺伝的アルゴリズムのスタート時の初期解として、各製品ジョブ内の加工工程順序を保したランダムな遺伝子列に導入する方法で生成する。
...
GAの適応度評価は、各染色体が表現するスケジュールの品質，ここではJSSPにおける代表的な性能指標である総加工時間を評価基準として採用する。
...
対応するスケジュールについて甘特図\todo[inline]{意味不明,ガントチャート？}と各ジェネレーション世代にもっとも短い総加工時間の折れ線グラフを描画し、視覚的に評価の適切さを確認できる。
...
\begin{enumerate}
    ...
    \end{enumerate}
記号：
\todo[inline]{操作と記号が混合している表記で非常に気持ちが悪い}
\begin{itemize}
    ...
    \item 親個体を頭に子個体に未配置の操作列：$P_{1}o \in [o_1^1, o_2^1, \dots, o_\theta^1] , P_{1}o \notin C $ 、 $P_{2}o \in [o_1^2, o_2^2, \dots, o_\theta^2] , P_{2}o \notin C $
    \item 子個体$C$の初期値の空列：$C \leftarrow [\ ]$
    \end{itemize}

\todo[inline]{以下はdescriptionで記述,「以上を用いた交差配列の手続きは以下のようになる。」，を入れるほうが良い}
\begin{description}
...
\subsection{GAの実験結果}
以上の手順で121個データをスケジュールして、最後の許容解総加工時間とガントチャートを結果として出力した。

\todo[inline]{p.19の図５はどう見ても表． また小さすぎる，注文のタイプを大，並，小としたはず）}
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.5\linewidth]{figures/GA_result.png}
  \caption{GAが出力した121ジョブセット（注文タイプ別）総加工時間}
  \label{fig:GA result}
\end{figure}
...
\todo[inline]{一般注文，大きい注文とか，用語や呼び方を統一してください．注文（大，並，小）ならそれに統一する．ちなみに日本語で並，は牛丼屋しかつかわない，ふつうは大中小と記述する．特に学術論文に「並」は使わないほうがよい．}
...
\begin{subfigure}{\linewidth}
    ...
    \caption{注文書19(小さい注文)のジェネレーション折れ線グラフ}
    \label{fig:19 generation}
\end{subfigure}
\vspace{1.5em} % 上下间距，可调
\begin{subfigure}{\linewidth}
    \centering
    \includegraphics[width=0.9\linewidth]{figures/gantt_19.png}
    \caption{注文書19(注文（小）)の許容解ガントチャート}
    \label{fig:19 gantt}
\end{subfigure}

\caption{注文書19(注文（小）)の実験結果}
...
\begin{subfigure}{\linewidth}
    ...
    \caption{注文書68(注文（中）)が生成した折れ線グラフ}
    \label{fig:68 generation}
\end{subfigure}
\vspace{1.5em}
\begin{subfigure}{\linewidth}
    \centering
    \includegraphics[width=0.9\linewidth]{figures/gantt_68.png}
...
\begin{subfigure}{\linewidth}
    ...
    \caption{注文書93(大きい注文)のジェネレーション折れ線グラフ}
    \label{fig:93 generation}
\end{subfigure}
\vspace{1.5em}
\begin{subfigure}{\linewidth}
    \centering
    \includegraphics[width=0.9\linewidth]{figures/gantt_93.png}
...
\clearpage
GAの実験結果を見て、ジェネレーション回数は50回以下で、200回の計算終了時と変わらない最適解を得ていることがわかる。収束まで最も計算回数の多いケースでも200回以下で良いスケジューリング案に到達している。
ガントチャートからは、加工工程のスケジューリングの可視化結果から，機械の遊休時間がほとんど見られないスケジュール案が出力できることも視認できる。
...
強化学習は、環境との相互作用を通じて逐次的に意思決定を行う枠組みであり、
...
図\ref{\label{fig:dqn}}に DQN の仕組みとして手法の概要を示す。また、表\ref{tab:rl_notation}に使用する記号をまとめる。

エージェントは時刻 $t$ で環境の状態 $s_t \in S$ を観測し、
...
すべての作業工程が完了までを指す。
すなわち、1 episode は
一つのジョブセットに対する完全なスケジューリング過程に対応する。
\todo[inline]{この完全の意味がよくわからない．一つの許容可能なmakespanを有するスケジュール案作成，という意味？}
...
は、リプレイバッファ(replay buffer)に保存される。
学習時には、
...
\begin{figure}[htbp]
  \centering
  \includegraphics[width=0.8\linewidth]{figures/wait_v3_1_graph.png}
...
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.8\textheight]{figures/wait_v5_2_graph.png}
  }
...
前章に述べた3タイプの注文(小、並、大)ごとに対して,
複合型報酬関数でのDQNと中心型報酬関数でのDQN両方をGAと比較する。
複合型報酬関数でのDQNで得られた解はほどんどGAに超えられないので,
3タイプの注文ごとにそれぞれGAと比べて近い例(比率1.3ぐらい)、離れた例(比率1.5ぐらい)とだいぶ離れた例(比率1.8ぐらい)を示す.
中心型報酬関数でのDQNで得られた解はGAと互角しているので、
3タイプの注文ごとにそれぞれGAを超える例(比率0.9ぐらい)、GA相当な例(比率1ぐらい)とGAを劣る例(比率1.2ぐらい)を示す.
\clearpage

\subsubsection{複合型報酬関数での実験結果}
図\ref{fig:complex reward vs ga result 1.34(small)}は注文(小)タイプに対して,
複合型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.34の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/complex_1.34(small).png}
  }
...
複合型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.48の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/complex_1.48(small).png}
  }
...
複合型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.76の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/complex_1.76(small).png}
  }
...
図\ref{fig:complex reward vs ga result 1.2(regular)}は注文(並)タイプに対して,
複合型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.2の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/complex_1.2(regular).png}
  }
  \caption{注文(並)複合型報酬関数DQNとGAのガントチャート(比率1.2)}
  \label{fig:complex reward vs ga result 1.2(regular)}
\end{figure}
\clearpage

図\ref{fig:complex reward vs ga result 1.5(regular)}は注文(並)タイプに対して,
複合型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.5の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/complex_1.5(regular).png}
  }
  \caption{注文(並)複合型報酬関数DQNとGAのガントチャート(比率1.5)}
  \label{fig:complex reward vs ga result 1.5(regular)}
\end{figure}
\clearpage

図\ref{fig:complex reward vs ga result 1.72(regular)}は注文(並)タイプに対して,
複合型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.72の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/complex_1.72(regular).png}
  }
  \caption{注文(並)複合型報酬関数DQNとGAのガントチャート(比率1.72)}
  \label{fig:complex reward vs ga result 1.72(regular)}
\end{figure}
\clearpage

図\ref{fig:complex reward vs ga result 1.22(large)}は注文(大)タイプに対して,
複合型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.22の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/complex_1.22(large).png}
  }
...
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/complex_1.5(large).png}
  }
...
複合型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.71の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/complex_1.71(large).png}
  }
...
図群に示す複合型報酬関数を用いた DQN のスケジューリング結果では，
GA により得られた解と比較して$makespan$ が大きく悪化することが確認された。
特に，工序数が多い Job5 および Job6 に処理が集中し，
他ジョブの実行可能な工序が十分に挿入されないスケジュール構造が共通して観測される。

この要因の一つとして，
本研究で設計した複合型報酬関数における即時報酬と将来報酬の相対的な影響の偏りが挙げられる。
本環境では，残作業量が最大のジョブを優先する項が強く作用しており，
Job5 および Job6 を連続的に選択する行動系列は，
将来の報酬を含めた行動価値として高く評価されやすい。
その結果，ある時点で Job5 または Job6 を選択した後は，
同一ジョブを継続して処理する行動が高い将来報酬を見込める選択として学習される。
...
既に形成された行動系列に基づく将来報酬と比較しても行動価値が低く評価される傾向にある。
...
$makespan$中心型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が0.95の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/simple_0.95(small).png}
  }
...
$makespan$中心型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が0.99の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/simple_0.99(small).png}
  }
...
$makespan$中心型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.24の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/simple_1.24(small).png}
  }
...
図\ref{fig:simple reward vs ga result 0.97(regular)}は注文(並)タイプに対して,
$makespan$中心型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が0.97の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/simple_0.97(regular).png}
  }
  \caption{注文(並)$makespan$中心型報酬関数DQNとGAのガントチャート(比率0.97)}
  \label{fig:simple reward vs ga result 0.97(regular)}
\end{figure}
\clearpage

図\ref{fig:simple reward vs ga result 0.99(regular)}は注文(並)タイプに対して,
$makespan$中心型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が0.99の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/simple_0.99(regular).png}
  }
  \caption{注文(並)$makespan$中心型報酬関数DQNとGAのガントチャート(比率0.99)}
  \label{fig:simple reward vs ga result 0.99(regular)}
\end{figure}
\clearpage

図\ref{fig:simple reward vs ga result 1.2(regular)}は注文(並)タイプに対して,
$makespan$中心型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.2の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/simple_1.2(regular).png}
  }
  \caption{注文(並)$makespan$中心型報酬関数DQNとGAのガントチャート(比率1.2)}
  \label{fig:simple reward vs ga result 1.2(regular)}
\end{figure}
\clearpage

図\ref{fig:simple reward vs ga result 0.94(large)}は注文(大)タイプに対して,
$makespan$中心型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が0.94の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/simple_0.94(large).png}
  }
...
$makespan$中心型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/simple_1(large).png}
  }
...
$makespan$中心型報酬関数DQNで得られたガントチャートの$makespan$とGAで得られたガントチャートの$makespan$の比率が1.16の例.
\begin{figure}[htbp]
  \centering
  \rotatebox{90}{
    \includegraphics[width=0.7\textheight]{figures/simple_1.16(large).png}
  }
  \caption{注文(大)$makespan$中心型報酬関数DQNとGAのガントチャート(比率1)}
...
\subsection{実験考察}
以上の実験結果より，本研究で検討した二種類の報酬関数は，
いずれも DQN によるJSSPスケジューリング方策の学習に寄与する一方で,
異なる課題を含まれていることが明らかとなった。
...
過去の行動に基づく将来的報酬が過大に評価される場合，結果として総 $makespan$ を悪化させる調度が生成される可能性がある。
今後は，即時報酬と将来報酬の寄与度を適切に調整する仕組みや，
学習段階に応じて報酬構造を変化させる手法を導入することで，
GA と比較しても極端な解を生じにくい，より安定したスケジューリング方策の獲得が期待される。

第二に，
モデルの一般化性能の向上が課題として挙げられる。
本研究では，製造する製品の加工工程順序をあらかじめ固定した問題設定を採用した。
そのため，学習済みモデルは，訓練時と同一または類似した加工工程構造を持つ問題に対しては有効である一方，
訓練時に出現しなかった加工工程順序を持つ製品に対する適応性には制限がある。
