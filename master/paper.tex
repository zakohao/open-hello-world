...
\begin{itemize}
  ...
  \begin{equation}
  r^{prog}_t = \beta (g_{step+1}-g_{step}) .
  \end{equation}
  ...
  \begin{equation}
  r^{bal}_t = -\gamma {std}\left(\{t^{sum}_m\}\right).
  \end{equation}
  ...
  \begin{equation}
  r^{\mathrm{opp}}_t = -\eta (t_{s,q} - t^{A}_{min}).
  \end{equation}
  「すぐ終わる作業」の優先を促す報酬である．
  ...
  r^{\mathrm{final}}_t =
  \begin{cases}
  \kappa \dfrac{C^{LB}}{C_{\max}} & (\text{エピソード終端時}) \\
  0 & (\text{それ以外})
  \end{cases}.
  \end{equation}
...
この単純型報酬関数は目的関数 $C_{\max}$ と直接かかわっているため，
報酬設計によるバイアスが小さく，
