...
$OM_{s,q} = \{1,2,\dots,12,13\}$ は製品$s$の第$q$番目加工工程の加工機械は$OM_{s,q}$である。
...
この三つの加工工程を除いて、他すべての加工工程の加工時間は$n_{s} \times {単位時間}$の積である。
...
これにより、実際の生産現場に見られる受注量とロットサイズのランダムネスでデジタルとして生成する。
