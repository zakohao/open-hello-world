...
def train(training_datasets):
    ...
    for ep in tqdm(range(EPISODES), desc="训练进度"):
        ...
        while not done:
            ...
            # 存储到memory时使用CPU张量以减少GPU内存使用
            memory.append((state.cpu(), action, reward, next_state_tensor.cpu(), done))
    ...
    # 保存模型
    torch.save(model.state_dict(), 'GPU_R(p+u)_LR0.0001_3000ep_barch32_jssp_model_npcb_(10,2)_(3,1)_(13)_(3).pth')
    print(f"模型已保存")
